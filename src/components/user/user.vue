  ...
  <div>
    ...
    <el-dialog title="修改用户信息"
      ...
      <span slot="footer"
        ...
        <el-button type="primary"
                   @click="editUserInfo(editForm.id)">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    showEditDialog: async function (id) {
      ...
      if (res.meta.status !== 200) return this.$message.error('获取用户修改信息列表失败')
    ...
    editUserInfo: function (id) {
      this.$refs.editFormRef.validate(async (valid) => {
        ...
        if (!res.meta.status !== 200) return this.$message.error('修改用户失败:' + res.meta.msg)
        // 关闭对话框 提示更新成功 刷新数据
